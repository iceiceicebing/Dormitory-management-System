<<<<<<< Updated upstream:宿舍管理系统/manager.cpp
...
void ManagerInspectDorm() {
    ...
    for (i = 0; i < all_dorms.count; i++) {
        if (strcmp(all_dorms.data[i].dorm_building, building_input) == 0 &&
            ...
            if (all_dorms.data[i].all_scores.count < 10) {
                all_dorms.data[i].all_scores.scores[all_dorms.data[i].all_scores.count] = (float)score_input;
                all_dorms.data[i].all_scores.count++;
            }
            else {
                for (j = 0; j < 9; j++) {
                    all_dorms.data[i].all_scores.scores[j] = all_dorms.data[i].all_scores.scores[j + 1];
                }
                all_dorms.data[i].all_scores.scores[9] = (float)score_input;
            }
            printf("打分成功！该宿舍当前共有 %d 次查寝记录。\n", all_dorms.data[i].all_scores.count);
            SaveDormInfo(all_dorms);
            break;
...
// 发布公告（使用AnnounceInsert函数复用announce.cpp中的逻辑）
void ManagerPublishAnnounce(ManagerNode* manager) {
    AnnounceInfo announce_info;
    memset(&announce_info, 0, sizeof(AnnounceInfo));

    printf("\n===== 发布新公告 =====\n");
    printf("公告类型（0-重要通知，1-寻物启事，2-其他）：");
    if (scanf("%d", &announce_info.type) != 1) {
        printf("类型输入错误！\n");
        while (getchar() != '\n');
        return;
    }
    ...
    printf("请输入标题：");
    fgets(announce_info.heading, sizeof(announce_info.heading), stdin);
    announce_info.heading[strcspn(announce_info.heading, "\n")] = '\0';
    ...
    strcpy(announce_info.content, content);

    strcpy(announce_info.sender, manager->data.name);
    printf("请输入发布日期（字符串，格式如20260603）：");
    fgets(announce_info.time, sizeof(announce_info.time), stdin);
    announce_info.time[strcspn(announce_info.time, "\n")] = '\0';

    // 使用AnnounceInsert函数将公告插入链表
    if (AnnounceInsert(announcement_board, 1, announce_info) == OK) {
        SaveAnnounceList(announcement_board);
        printf("公告发布成功\n");
    } else {
        printf("公告发布失败\n");
...
void ManagerSystem(ManagerNode* manager) {
    ...
    while (1) {
        ...
        case 4:  // 发布公告
            ManagerPublishAnnounce(manager);
            break;

        case 5:  // 查看所有公告
            ManagerViewAllAnnounces();
            break;
...
>>>>>>> Stashed changes:宿舍管理系统/宿舍管理系统/manager.cpp
}