<<<<<<< Updated upstream:宿舍管理系统/dorm_system.h
...
static int CompareDormByAvg(const void* a, const void* b);//比较函数
Status ScoreDorm(StudentNode* union_member);      // 卫生打分与录入
void GetCurrentDate(char* dateStr);// 获取当前日期，格式为YYYYMMDD
Status ShowDormRanking(StudentNode* union_member); // 查看宿舍排名
Status StatByBuilding();              // 按楼栋统计人数
Status StatByTime();                  // 按时间统计在寝情况
Status ExportReport();                // 导出报表
...
void ManagerSystem(ManagerNode* manager); // 宿管端具体操作
// 宿管功能模块函数声明
void ManagerInspectDorm(); // 查房打分
void ManagerApproveAlter(); // 审批换宿申请
void ManagerApproveExit(); // 审批退宿申请
void ManagerPublishAnnounce(ManagerNode* manager); // 发布公告
void ManagerViewAllAnnounces(); // 查看所有公告
void ManagerViewDormInfo(); // 查看宿舍信息
// 之后的操作函数声明可以写在这后面……
...
void TeacherSystem(TeacherList& L, TeacherNode* teacher); // 辅导员端具体操作
// 之后的操作函数声明可以写在这后面……
...
>>>>>>> Stashed changes:宿舍管理系统/宿舍管理系统/dorm_system.h
